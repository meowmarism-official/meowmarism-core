...
const version = (over = {}) => ({
  id: 'v1', project_id: 'p1', name: 'Pack 1.0', version_number: '1.0', version_type: 'release', game_versions: ['1.21.1'], loaders: ['neoforge', 'mrpack'],
...
test('search asks for server-capable modpacks and maps the hits', async () => {
  ...
  assert.deepEqual(JSON.parse(url.searchParams.get('facets')), [['project_type:modpack'], ['server_side!=unsupported'], ['categories:neoforge'], ['versions:1.21.1']]);
});
...
test('search ignores unknown sorts and loaders and rejects odd Minecraft versions', async () => {
  ...
  assert.deepEqual(JSON.parse(url.searchParams.get('facets')), [['project_type:modpack'], ['server_side!=unsupported']]);
